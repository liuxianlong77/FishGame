...
MenuSelect::MenuSelect()
{
    ...
    Help::AddOneByOneTouch(this->getEventDispatcher(), this, true, [this](CC::Touch* touch, CC::Event* event){
        /*for (auto& item : mItems) {
            CC::Rect itemBox = CC::Rect(this->getPositionX(),this->getPositionY(),item->getContentSize().width,item->getContentSize().height);
            isMove = itemBox.containsPoint(touch->getLocation());
            if (isMove) {
                break;
            }
        }
...
void MenuSelect::selectLevel(int index)
{
    if (isMove) return;
    CCLOG("%d", index);
    if (mCurrentItem == index) /*select level*/;
    mCurrentItem = index;
    //select level
}
